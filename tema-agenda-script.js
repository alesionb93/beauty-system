...
(function() {
  ...
  var BOOKING_FIELDS = {
    ...
    'bk-step-active':'step_active','bk-step-done':'step_done','bk-step-inactive':'step_inactive','bk-step-text':'step_text','bk-stepper-line':'stepper_line',
  ...
  window.onTemaLogoChange = function(input) {
    ...
    reader.onload = function(e) {
      ...
      var icon = document.getElementById('tema-logo-icon');
      var removeBtn = document.getElementById('btn-remover-tema-logo');
      currentTheme.logo_url = e.target.result; // fallback local/preview até o upload concluir
  ...
  /* ══════════════════════════════════════════════
     SALVAR TEMA
     ══════════════════════════════════════════════ */
  function montarTemaCompleto(tenantId, logoUrl) {
    var accent = currentTheme.gold || DEFAULTS.gold;
    var vars = generateAccentVariations(accent);
    return {
      tenant_id: tenantId,
      gold: accent,
      ...
      sidebar_bg: currentTheme.sidebar_bg || DEFAULTS.sidebar_bg,
      ...
      logo_url: logoUrl || currentTheme.logo_url || null,
      ...
      page_title_color: currentTheme.page_title_color || currentTheme.text_color || DEFAULTS.page_title_color,
      booking_theme: Object.assign({}, BOOKING_DEFAULTS, currentBookingTheme),
      updated_at: new Date().toISOString()
    };
  }

  function persistirTemaLocal(tenantId, theme) {
    try {
      localStorage.setItem('agenda_theme:' + tenantId, JSON.stringify(theme));
      console.log('[tema] tema completo salvo no localStorage para sync local');
    } catch(e) { console.warn('[tema] falha ao salvar no localStorage:', e); }
    try {
      if (typeof BroadcastChannel !== 'undefined') {
        var bc = new BroadcastChannel('beauty-theme-sync');
        bc.postMessage({ type: 'theme-updated', tenantId: tenantId, theme: theme });
        setTimeout(function(){ try { bc.close(); } catch(e){} }, 250);
      }
    } catch(e) { console.warn('[tema] falha ao emitir BroadcastChannel:', e); }
  }
  ...
  window.salvarTema = async function() {
    ...
    var theme = montarTemaCompleto(tenantId, logoUrl);

    // Persistência local obrigatória: garante o link externo no ambiente local,
    // mesmo quando config.js/Supabase/RLS/colunas locais estiverem divergentes da produção.
    persistirTemaLocal(tenantId, theme);

    currentTheme = Object.assign({}, theme);
    currentBookingTheme = Object.assign({}, BOOKING_DEFAULTS, theme.booking_theme || {});
    aplicarTemaNoAgenda(currentTheme);
    updateAllPreviews();
    updateBookingPreview();

    if (typeof supabaseClient === 'undefined') {
      mostrarFeedback('Tema salvo localmente. O link externo local já consegue ler estas configurações.', true);
      return;
    }
    ...
    if (resp.error) {
      ...
      if (msg.indexOf('booking_theme') !== -1) {
        mostrarFeedback('Tema salvo localmente, mas falta a coluna booking_theme no banco local. Rode o SQL de migração.', false);
        return;
      }
      if (msg.indexOf('appt_') !== -1 || msg.indexOf('page_title') !== -1 || msg.indexOf('modal_bg') !== -1 || msg.indexOf('input_bg') !== -1 || msg.indexOf('text_muted') !== -1) {
        mostrarFeedback('Tema salvo localmente, mas faltam colunas novas no banco local. Rode o SQL de migração.', false);
        return;
      }
      mostrarFeedback('Tema salvo localmente, mas houve erro ao salvar no banco: ' + msg, false);
      return;
    }
  ...
  /* ══════════════════════════════════════════════
     INIT
     ══════════════════════════════════════════════ */
  // ── BroadcastChannel: responde a pedidos de tema do agendamento-cliente ──
  try {
    if (typeof BroadcastChannel !== 'undefined') {
      var themeSyncChannel = new BroadcastChannel('beauty-theme-sync');
      themeSyncChannel.onmessage = function(ev) {
        if (ev.data && ev.data.type === 'theme-request') {
          var reqTenant = ev.data.tenantId;
          var myTenant = localStorage.getItem('currentTenantId');
          if (reqTenant === myTenant && currentTheme) {
            themeSyncChannel.postMessage({
              type: 'theme-response',
              tenantId: reqTenant,
              theme: montarTemaCompleto(reqTenant, currentTheme.logo_url)
            });
          }
        }
      };
    }
  } catch(e) { console.warn('[tema] BroadcastChannel sync falhou:', e); }
