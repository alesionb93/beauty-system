...
try {
  if (/icons\/Beauti-System/i.test(location.pathname)) {
    console.warn('[DEPRECATED PATH] Página servida de icons/Beauti-System (pasta removida). Use a raiz do projeto.');
  }
} catch (e) {}

(function () {
  function cleanAssetPath(file) {
    return String(file || '').replace(/^\/+/, '');
  }
  window.__beautyAssetUrl = window.__beautyAssetUrl || function (file) {
    try {
      var clean = cleanAssetPath(file);
      var current = document.currentScript && document.currentScript.src ? document.currentScript.src : '';
      var scriptTag = current ? null : document.querySelector('script[src$="/script.js"],script[src="script.js"],script[src$="script.js"]');
      var base = window.__BEAUTY_ASSET_BASE__ || (current ? current.replace(/[^/]*$/, '') : (scriptTag && scriptTag.src ? scriptTag.src.replace(/[^/]*$/, '') : '/'));
      return new URL(clean, base).href;
    } catch (e) {
      return '/' + cleanAssetPath(file);
    }
  };
})();
...
(function () {
  ...
  function ensureAudio() {
    if (!__earlyAudio) {
      try {
        __earlyAudio = new Audio(window.__beautyAssetUrl ? window.__beautyAssetUrl('sounds/appointment.mp3') : '/sounds/appointment.mp3');
...
(function beautyAgendaRealtimeModule() {
  ...
  function notifyNovoAgendamento(nomeCli, agId, origem) {
    ...
    // Toast e som são independentes: falha em um nunca bloqueia o outro.
    try {
      if (typeof window.showToast === 'function') window.showToast(msg, 'success');
    } catch (errToast) {
      console.error('[realtime] showToast falhou:', errToast);
    }
    try {
      if (typeof window.playAppointmentSound === 'function') window.playAppointmentSound();
    } catch (errSound) {
      console.error('[realtime] playAppointmentSound falhou:', errSound);
    }
  }
  ...
  function handleNovoAgendamentoEvent(data, origem) {
    ...
    if (data.tenantId && meuTenant && String(data.tenantId) !== String(meuTenant)) {
      console.log('[' + origem + '] ignorado (outro tenant)');
      return;
    }
    console.log('🔔 EVENTO BRUTO (' + origem + '):', data);
    notifyNovoAgendamento(data.cliente_nome, data.agendamento_id, origem);
    refetchAgendaNow(origem + ':novo-agendamento');
  }
  window.handleNovoAgendamentoEvent = handleNovoAgendamentoEvent;
  ...
  try {
    if (typeof BroadcastChannel !== 'undefined') {
      ...
      __bc.onmessage = function (ev) {
        handleNovoAgendamentoEvent(ev && ev.data, 'bcast');
      };
  ...
  // Fallback por localStorage: cobre navegadores/ambientes onde BroadcastChannel falha.
  try {
    window.addEventListener('storage', function (event) {
      if (!event || event.key !== 'beauty-agenda:last-event' || !event.newValue) return;
      try {
        handleNovoAgendamentoEvent(JSON.parse(event.newValue), 'storage');
      } catch (err) {
        console.warn('[realtime] storage event inválido:', err);
      }
    });
  } catch (e) {
    console.warn('[realtime] fallback storage indisponível:', e);
  }
...
async function applyPermissions() {
  ...
  if (role === 'master_admin') {
    ...
    /* ============================================================
       MULTI-TENANT (v2 - 2026-06-18 - via tenant_groups)
       admin/colaborador:
         1) Tenant principal = usuarios.tenant_id (fallback: 1º user_roles)
         2) Se principal está em tenant_group_tenants -> usa todos
            os tenants do grupo
         3) Senão -> retrocompat: lista de user_roles
         4) Decide igual antes: 0/1 entra direto, 2+ vai p/ select-unidade
       ============================================================ */
    var sess = await supabaseClient.auth.getSession();
    var sessionObj = sess && sess.data ? sess.data.session : null;
    var rolesRows = [];
    var principalTenantId = null;

    if (sessionObj) {
      var userResp = await supabaseClient
        .from('usuarios')
        .select('tenant_id')
        .eq('id', sessionObj.user.id)
        .maybeSingle();
      if (userResp.data && userResp.data.tenant_id) {
        principalTenantId = userResp.data.tenant_id;
      }

      var rolesResp = await supabaseClient
        .from('user_roles')
        .select('tenant_id')
        .eq('user_id', sessionObj.user.id);
      if (!rolesResp.error && rolesResp.data) rolesRows = rolesResp.data;

      if (!principalTenantId) {
        for (var i = 0; i < rolesRows.length; i++) {
          if (rolesRows[i] && rolesRows[i].tenant_id) { principalTenantId = rolesRows[i].tenant_id; break; }
        }
      }
    }

    var tenantIdsDoUsuario = [];

    // v3: usa RPC SECURITY DEFINER para expandir tenants via grupo
    if (sessionObj) {
      var rpcResp = await supabaseClient.rpc('get_user_tenant_ids', { _user_id: sessionObj.user.id });
      if (!rpcResp.error && Array.isArray(rpcResp.data)) {
        rpcResp.data.forEach(function(row) {
          var tid = row && (row.tenant_id || row.TENANT_ID);
          if (tid && tenantIdsDoUsuario.indexOf(tid) === -1) tenantIdsDoUsuario.push(tid);
        });
      } else if (rpcResp.error) {
        console.warn('[multi-tenant] RPC get_user_tenant_ids falhou, usando fallback:', rpcResp.error);
      }
    }

    // Fallback retrocompatível (RPC indisponível): user_roles + principal
    if (tenantIdsDoUsuario.length === 0) {
      rolesRows.forEach(function(r) {
        if (r && r.tenant_id && tenantIdsDoUsuario.indexOf(r.tenant_id) === -1) {
          tenantIdsDoUsuario.push(r.tenant_id);
        }
      });
      if (principalTenantId && tenantIdsDoUsuario.indexOf(principalTenantId) === -1) {
        tenantIdsDoUsuario.push(principalTenantId);
      }
    }

    window.__userTenantIds = tenantIdsDoUsuario.slice();

    var storedTenant = localStorage.getItem('currentTenantId');
    var tenantEscolhido = null;

    if (tenantIdsDoUsuario.length >= 2) {
      if (storedTenant && tenantIdsDoUsuario.indexOf(storedTenant) !== -1) {
        tenantEscolhido = storedTenant;
      } else {
        window.location.href = 'select-unidade.html';
        return false;
      }
    } else if (tenantIdsDoUsuario.length === 1) {
      tenantEscolhido = tenantIdsDoUsuario[0];
    } else {
      // Retrocompat total: nem grupo nem user_roles -> usa principal
      tenantEscolhido = principalTenantId || (await getUserTenant());
    }

    currentUser.tenantId = tenantEscolhido;
    localStorage.setItem('currentTenantId', tenantEscolhido || '');
  }
  ...
  /* ============================================================
     MULTI-TENANT (v1 - 2026-06-18)
     Para admin/colaborador com 2+ unidades: adicionar opção
     "Trocar Unidade" no menu (sem necessidade de logout).
     ============================================================ */
  if (role !== 'master_admin') {
    var multiTenants = (window.__userTenantIds || []);
    if (multiTenants.length >= 2) {
      var navArea2 = document.querySelector('.sidebar-nav');
      if (navArea2 && !document.getElementById('btn-trocar-unidade')) {
        var btn2 = document.createElement('button');
        btn2.className = 'nav-btn';
        btn2.id = 'btn-trocar-unidade';
        btn2.innerHTML = '<i class="fa-solid fa-shop"></i> Trocar Unidade';
        btn2.onclick = function() {
          // Limpa apenas o tenant ativo; sessão de auth permanece intacta.
          try { localStorage.removeItem('currentTenantId'); } catch (e) {}
          try { localStorage.removeItem('currentTenantNome'); } catch (e) {}
          window.location.href = 'select-unidade.html';
        };
        navArea2.appendChild(btn2);
      }
      // Exibe o nome da unidade atual na sidebar, se disponível
      var tenantNome2 = localStorage.getItem('currentTenantNome') || '';
      if (tenantNome2) {
        var brand2 = document.querySelector('.sidebar-header .brand p');
        if (brand2) brand2.textContent = tenantNome2;
      }
    }
  }
...
async function loadAppointments() {
  ...
  var query = supabaseClient.from('agendamentos').select('*, agendamento_pagamentos(id, forma_pagamento, valor, parcelas, created_at), agendamento_servicos(id, created_at, servico_id, profissional_id, preco, duracao, cor_id, cliente_pacote_id, origem, credito_consumido, servicos(id, nome, preco, duracao), cores(id, nome, hex), agendamento_servico_cores(id, cor_id, tipo, quantidade, cores(id, nome, hex)))');
  if (tenantId) query = query.eq('tenant_id', tenantId);
  var resp = await query;
  if (resp.error) {
    console.warn('Erro agendamentos com pagamentos embutidos, tentando fallback:', resp.error);
    var fallbackQuery = supabaseClient.from('agendamentos').select('*, agendamento_servicos(id, created_at, servico_id, profissional_id, preco, duracao, cor_id, cliente_pacote_id, origem, credito_consumido, servicos(id, nome, preco, duracao), cores(id, nome, hex), agendamento_servico_cores(id, cor_id, tipo, quantidade, cores(id, nome, hex)))');
    if (tenantId) fallbackQuery = fallbackQuery.eq('tenant_id', tenantId);
    resp = await fallbackQuery;
  }
  ...
  appointments = resp.data.map(function(a) {
    ...
    var pagamentosAg = Array.isArray(a.agendamento_pagamentos) ? a.agendamento_pagamentos : [];
    var valorTotalPagoCalc = pagamentosAg.reduce(function(total, p) {
      return total + (Number(p && p.valor) || 0);
    }, 0);
    var statusPagamentoRaw = String(a.status_pagamento || a.payment_status || '').toLowerCase();
    var statusPagamentoNorm = (statusPagamentoRaw === 'paid') ? 'pago' : statusPagamentoRaw;
    var possuiPagamentoCalc = !!a.possui_pagamento || valorTotalPagoCalc > 0 || Number(a.valor_total_pago || 0) > 0 || statusPagamentoNorm === 'pago' || statusPagamentoNorm === 'parcial';
    // 🔧 PACOTE-FIX (BUG INTEGRIDADE): IDs de cliente_pacote que ESTE agendamento
    // gerou (origem='pacote_venda'). Usado para:
    //  (a) exibir badge "Venda de pacote" no bloco da agenda.
    //  (b) bloquear exclusão enquanto existirem outros agendamentos consumindo
    //      esses créditos (evita saldo fantasma e estados órfãos).
    var _vendaPacoteIds = [];
    (vendaItems || []).forEach(function(v){
      if (v && v.cliente_pacote_id && _vendaPacoteIds.indexOf(v.cliente_pacote_id) === -1) {
        _vendaPacoteIds.push(v.cliente_pacote_id);
      }
    });
    return {
      id: a.id,
      ...
      prepaid: a.prepaid === true,
      status_pagamento: statusPagamentoNorm || null,
      payment_status: a.payment_status || null,
      valor_total_pago: Number(a.valor_total_pago || 0) || valorTotalPagoCalc || 0,
      possui_pagamento: possuiPagamentoCalc,
      pagamentos: pagamentosAg,
      prepaid_origin_agendamento_id: a.prepaid_origin_agendamento_id || null,
      prepaid_origin_payment_id: a.prepaid_origin_payment_id || null,
      servicos: svcs.length > 0 ? svcs : null,
      _pacoteVendaItems: vendaItems,
      _hasVendaPacote: vendaItems.length > 0,
      _vendaPacoteIds: _vendaPacoteIds
    };
  });
}
...
async function loadServicos() {
  ...
  allServicos.forEach(function(s) {
    // 🔧 PRÉ-PAGO: também expõe `id` para que o add-on consiga inserir em agendamento_servicos
    servicePrices[s.nome] = { id: s.id, preco: parseFloat(s.preco), duracao: s.duracao };
  });
...
async function insertClient(clientObj) {
  ...
  var telefoneFormatado = normalizeTelefoneBR(clientObj.telefone);
...
async function openEditClienteModal(clienteId) {
  ...
  if (telEl) {
    // Reconstrói máscara visual "+55 (XX) XXXXX-XXXX" a partir do valor salvo no banco
    telEl.value = formatTelefoneProfDisplay(cliente.telefone || '');
    try { maskTelefoneProf(telEl); } catch(e) {}
  }
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  // PADRÃO OFICIAL: telefone do cliente usa o MESMO mask do profissional (+55 chumbado)
  maskTelefoneProf(document.getElementById('id-telefone'));
  maskTelefoneProf(document.getElementById('cl-telefone'));
...
/* ===== PHONE NORMALIZATION (BR / WhatsApp) ===== */
// Retorna apenas dígitos prefixados com 55 (ex: 5548991203769). Pronto para WhatsApp/API.
function normalizeTelefoneBR(tel) {
  var d = String(tel == null ? '' : tel).replace(/\D/g, '');
  if (!d) return '';
  // Se já vier com 55 + (10 ou 11) dígitos, mantém
  if ((d.length === 12 || d.length === 13) && d.indexOf('55') === 0) return d;
  // Caso usuário tenha digitado só DDD + número (10 ou 11 dígitos) -> prefixa
  if (d.length === 10 || d.length === 11) return '55' + d;
  // Fallback: garante prefixo 55 sem duplicar
  if (d.indexOf('55') === 0) return d;
  return '55' + d;
}

// Formata para exibição com prefixo fixo +55 (XX) XXXXX-XXXX
// IMPORTANTE: o prefixo "+55 " é apenas visual; aqui SEMPRE removemos qualquer "55" inicial
// dos dígitos para evitar duplicação (ex: bug "+55 (55").
function formatTelefoneProfDisplay(tel) {
  var d = String(tel == null ? '' : tel).replace(/\D/g, '');
  // Remove prefixo 55 se presente (independente do tamanho)
  if (d.indexOf('55') === 0) d = d.substring(2);
  if (d.length > 11) d = d.substring(0, 11);
  var out = '+55 ';
  if (d.length === 0) return out;
  if (d.length <= 2) return out + '(' + d;
  if (d.length <= 6) return out + '(' + d.substring(0,2) + ') ' + d.substring(2);
  if (d.length <= 10) return out + '(' + d.substring(0,2) + ') ' + d.substring(2,6) + '-' + d.substring(6);
  return out + '(' + d.substring(0,2) + ') ' + d.substring(2,7) + '-' + d.substring(7);
}

// Aplica máscara com prefixo +55 fixo (não editável) no input do profissional.
// Usa flag para não registrar listeners duplicados se chamado múltiplas vezes.
function maskTelefoneProf(input) {
  if (!input) return;
  function apply() {
    var formatted = formatTelefoneProfDisplay(input.value);
    if (input.value !== formatted) input.value = formatted;
    // Mantém o cursor sempre após a parte editável
    try {
      var len = input.value.length;
      input.setSelectionRange(len, len);
    } catch(e) {}
  }
  if (!input.dataset.maskProfBound) {
    input.addEventListener('focus', function() {
      var digits = input.value.replace(/\D/g, '').replace(/^55/, '');
      if (!digits) input.value = '+55 ';
      // Cursor sempre após "+55 "
      try {
        var pos = input.value.length;
        input.setSelectionRange(Math.max(pos, 4), Math.max(pos, 4));
      } catch(e) {}
    });
    input.addEventListener('input', apply);
    input.addEventListener('keydown', function(e) {
      var pos = input.selectionStart || 0;
      // Impede apagar/editar o prefixo fixo "+55 " (4 chars)
      if ((e.key === 'Backspace' && pos <= 4) || (e.key === 'Delete' && pos < 4)) {
        e.preventDefault();
        try { input.setSelectionRange(4, 4); } catch(err) {}
      }
    });
    input.addEventListener('click', function() {
      var pos = input.selectionStart || 0;
      if (pos < 4) { try { input.setSelectionRange(4, 4); } catch(e) {} }
    });
    input.dataset.maskProfBound = '1';
  }
  // Aplica formatação inicial sempre (reset garantido ao reabrir modal)
  apply();
}
...
function formatTelefoneDisplay(tel) {
  ...
  // Novo padrão: telefone salvo com prefixo 55 (12 ou 13 dígitos) -> exibir "+55 (DD) ....-...."
  if ((d.length === 12 || d.length === 13) && d.indexOf('55') === 0) {
    var rest = d.substring(2);
    if (rest.length === 11) return '+55 (' + rest.substring(0,2) + ') ' + rest.substring(2,7) + '-' + rest.substring(7);
    if (rest.length === 10) return '+55 (' + rest.substring(0,2) + ') ' + rest.substring(2,6) + '-' + rest.substring(6);
  }
  // Compatibilidade com clientes antigos (sem prefixo 55)
...
async function consultarCliente() {
  ...
  // Valida ignorando o prefixo "55" do mask (+55 chumbado)
  if (!tel || tel.replace(/\D/g,'').replace(/^55/, '').length < 10) {
  ...
  if (found) {
    ...
    setTimeout(function() {
      closeModal('modal-identificacao');
      document.getElementById('form-cliente').reset();
      var clTel = document.getElementById('cl-telefone');
      clTel.value = formatTelefoneProfDisplay(tel);
      try { maskTelefoneProf(clTel); } catch(e) {}
      openModal('modal-cliente');
    }, 1000);
  }
}
...
function appointmentMatchesFilter(a) {
  // 🟣 Oculta TODOS os agendamentos cancelados (status: cancelado, excluido,
  // desmarcado e cancelado_com_venda). O registro permanece no banco para
  // auditoria/estoque, mas NÃO ocupa horário visualmente — o slot fica livre.
  ...
  apps.forEach(function(a) {
    ...
    servicos.forEach(function(s, idx) {
      ...
      events.push({
        ...
        status: a.status,
        // 🎁 PACOTE-FIX: indicadores visuais de pacote (USO e/ou VENDA)
        // Um mesmo agendamento pode ter ambos: a venda do pacote E o 1º uso.
        // Mostramos os dois badges quando aplicável.
        isPackage: (s && (s.origem === 'pacote_uso' || !!s.cliente_pacote_id)) || false,
        isPackageSale: (idx === 0) && !!a._hasVendaPacote
      });
      cursor += dur;
    });
  });
...
/* ============================================================
   📐 POSICIONAMENTO VERTICAL DA AGENDA
   HOUR_HEIGHT = altura (px) de 1 hora cheia na timeline.
   Deve casar EXATAMENTE com a CSS de .timeline-row e
   .multi-agenda-row (estilos.css). Hoje = 60px → 1min = 1px.
   Fórmula canônica de posicionamento:
     top = (hours - hStart) * HOUR_HEIGHT + (minutes / 60) * HOUR_HEIGHT
   Assim, 18:30 NUNCA é empurrado para a linha das 19:00 —
   ocupa a metade inferior da célula das 18h.
   ============================================================ */
var HOUR_HEIGHT = 60;
function computeTopPx(hourNum, minNum, hStart) {
  return (hourNum - hStart) * HOUR_HEIGHT + (minNum / 60) * HOUR_HEIGHT;
}
function computeHeightPx(durationMin) {
  return Math.max((durationMin / 60) * HOUR_HEIGHT, 1);
}

/* 🧱 FIX ESTRUTURAL DA AGENDA
   Garante que:
   - O container pai dos eventos é position:relative
   - A "camada" de blocos (.timeline-blocks / .multi-agenda-blocks) é
     position:absolute alinhada ao TOPO da primeira row de horário
   - As rows não fazem overflow:hidden (não recortam blocos que
     começam em minutos fracionados como 18:30)
   - Cada bloco é position:absolute (não depende da row da hora cheia)
   Isto torna o posicionamento vertical puramente baseado em pixels
   calculados (top em px), independente da hora visual onde começa. */
function applyTimelineLayerLayout(container) {
  if (!container) return;
  // ---- MULTI AGENDA ----
  var body = container.querySelector('.multi-agenda-body');
  if (body) {
    body.style.position = 'relative';
    body.style.overflow = 'visible';
    var mRows = body.querySelectorAll('.multi-agenda-row');
    Array.prototype.forEach.call(mRows, function(r){
      r.style.overflow = 'visible';
      r.style.position = 'relative';
    });
    if (!mRows.length) return;
    var bodyRect = body.getBoundingClientRect();
    var firstRowTop = mRows[0].getBoundingClientRect().top - bodyRect.top;
    var layers = body.querySelectorAll('.multi-agenda-blocks');
    Array.prototype.forEach.call(layers, function(layer){
      // 🔧 FIX OVERFLOW HORIZONTAL (multi-profissionais):
      // Cada layer cobre APENAS a coluna do seu profissional.
      // Usa a célula correspondente da primeira row como referência de
      // left/width, em vez de esticar o layer por todo o body.
      var profName = layer.getAttribute('data-prof-col');
      var refCell = profName
        ? mRows[0].querySelector('.multi-agenda-cell[data-prof="' + profName.replace(/"/g, '\\"') + '"]')
        : null;
      layer.style.position = 'absolute';
      layer.style.top = firstRowTop + 'px';
      layer.style.bottom = 'auto';
      if (refCell) {
        var cellRect = refCell.getBoundingClientRect();
        layer.style.left = (cellRect.left - bodyRect.left) + 'px';
        layer.style.width = cellRect.width + 'px';
        layer.style.right = 'auto';
      } else {
        // Fallback: respeita o gutter de 60px da coluna de horários
        layer.style.left = '60px';
        layer.style.right = '0';
      }
      layer.style.height = (mRows.length * HOUR_HEIGHT) + 'px';
      layer.style.pointerEvents = 'none';
      // 🔒 Trava qualquer evento dentro dos limites da coluna do profissional
      layer.style.overflow = 'hidden';
      Array.prototype.forEach.call(layer.children, function(ch){
  ...
  // ---- SINGLE TIMELINE ----
  var tl = container.querySelector('.timeline');
  if (tl) {
    tl.style.position = 'relative';
    tl.style.overflow = 'visible';
    var tRows = tl.querySelectorAll('.timeline-row');
    Array.prototype.forEach.call(tRows, function(r){
      r.style.overflow = 'visible';
      r.style.position = 'relative';
    });
    var blocks = tl.querySelector('#timeline-blocks');
    if (!blocks || !tRows.length) return;
    var tlRect = tl.getBoundingClientRect();
    var firstSlot = tRows[0].querySelector('.timeline-slot');
    var firstRowTopT = tRows[0].getBoundingClientRect().top - tlRect.top;
    blocks.style.position = 'absolute';
    blocks.style.top = firstRowTopT + 'px';
    if (firstSlot) {
      var slotRect = firstSlot.getBoundingClientRect();
      blocks.style.left = (slotRect.left - tlRect.left) + 'px';
      blocks.style.width = slotRect.width + 'px';
      blocks.style.right = 'auto';
    } else {
      blocks.style.left = '0';
      blocks.style.right = '0';
    }
    blocks.style.height = (tRows.length * HOUR_HEIGHT) + 'px';
    blocks.style.pointerEvents = 'none';
    blocks.style.overflow = 'visible';
    Array.prototype.forEach.call(blocks.children, function(ch){
      ch.style.position = 'absolute';
      ch.style.pointerEvents = 'auto';
    });
  }
...
function renderSingleTimeline(container, dayEvents, horarioDia) {
  ...
  // 🧱 Força layout estrutural correto (parent relative + camada absoluta)
  applyTimelineLayerLayout(container);
}
...
function renderTimelineBlockMulti(container, ev, colIdx, totalCols, subCol, totalSubCols, hourStartOverride) {
  ...
  // 📐 Posição vertical considerando minutos (00, 15, 30, 45, …)
  var topPx = computeTopPx(hourNum, minNum, hStart);
  var duration = eventDuration(ev);
  var heightPx = computeHeightPx(duration);

  var endTime = computeEndTime(ev.hora, duration);

  var block = document.createElement('div');
  block.className = 'timeline-block';
  // 🔧 PRÉ-PAGO / PAGAMENTOS: expõe id do agendamento p/ injeção de badges
  if (ev.agendamento && ev.agendamento.id) {
    block.dataset.appointmentId = ev.agendamento.id;
    if (ev.agendamento.prepaid) block.classList.add('is-prepaid');
  }
  ...
  // O container (.multi-agenda-blocks[data-prof-col=...]) já é dimensionado
  // para cobrir SOMENTE a coluna do profissional (ver applyTimelineLayerLayout).
  // Portanto o cálculo aqui deve ser 100% RELATIVO a essa coluna — ignoramos
  // colIdx/totalCols (que se referem ao grid global) e usamos apenas
  // subCol/totalSubCols para eventos simultâneos do MESMO profissional.
  var subWidth = 100 / Math.max(1, totalSubCols);
  block.style.left = (subCol * subWidth) + '%';
...
function buildBlockContent(block, ev, heightPx, endTime, serviceNames) {
  ...
  var partTag = (ev.totalSvcs && ev.totalSvcs > 1)
    ...
    : '';
  // 🎁 PACOTE-FIX v13 (2026-05-07): badges visualmente DISTINTOS.
  // VENDA = âmbar com 💰 | USO = roxo com 🎟️.
  // Se ambos existem no mesmo agendamento, mostramos os DOIS.
  var usoBadge = ev.isPackage
    ? ' <span class="tb-pkg-badge tb-pkg-badge-uso" data-pkg-kind="uso" title="Uso de pacote (consome 1 crédito)">🎟️ Uso de pacote</span>'
    : '';
  var vendaBadge = ev.isPackageSale
    ? ' <span class="tb-pkg-badge tb-pkg-badge-venda" data-pkg-kind="venda" title="Venda de pacote (origem dos créditos)">💰 Venda de pacote</span>'
    : '';
  var pkgBadge = vendaBadge + usoBadge;
  if (ev.isPackage) block.classList.add('is-package');
  if (ev.isPackageSale) block.classList.add('is-package-sale');
  ...
  if (heightPx <= 22) {
    ...
    var tinyTitleParts = [];
    if (ev.isPackageSale) tinyTitleParts.push('[Venda de pacote]');
    if (ev.isPackage) tinyTitleParts.push('[Uso de pacote]');
    block.title = timeRange + ' — ' + ev.cliente + (serviceNames ? ' (' + serviceNames + ')' : '') + (tinyTitleParts.length ? ' ' + tinyTitleParts.join(' ') : '');
    var tinyDots = (ev.isPackageSale ? '<span class="tb-pkg-dot tb-pkg-dot-venda" title="Venda de pacote"></span>' : '')
                 + (ev.isPackage ? '<span class="tb-pkg-dot" title="Uso de pacote"></span>' : '');
    block.innerHTML = '<div class="tb-row-tiny"><span class="tb-time">' + ev.hora + '</span> <span class="tb-client tb-truncate">' + ev.cliente + '</span>' + tinyDots + '</div>';
  } else if (heightPx <= 38) {
...
function getExigirSenhaCancelamentoAtual() {
  if (!window.CANCELAMENTO_CFG) return true;
  return window.CANCELAMENTO_CFG.exigir_senha !== false;
}
...
async function sheetCancelarAgendamento() {
  var a = __currentSheetAppointment;
  if (!a) return;
  if (typeof window.CancelamentoAuth === 'undefined' || !window.CancelamentoAuth.solicitar) {
    showToast('Módulo de cancelamento não carregado.', 'error');
    return;
  }
  var tenantId = (typeof getCurrentTenantId === 'function') ? getCurrentTenantId() : (a.tenant_id || null);
  var possuiPag = !!a.possui_pagamento || Number(a.valor_total_pago || 0) > 0;
  closeAgendamentoBottomSheet();
  try {
    await window.CancelamentoAuth.solicitar({
      agendamentoId: a.id,
      tenantId: tenantId,
      possuiPagamento: possuiPag,
      exigirSenha: getExigirSenhaCancelamentoAtual(),
      onSuccess: async function(res) {
        showToast('Agendamento cancelado!' + (res && res.novo_status === 'cancelado_com_venda' ? ' (com venda)' : ''));
        await loadAppointments();
        renderCalendar();
        renderDayDetail();
        if (typeof rodarPagamentosDashboard === 'function') {
          try { await rodarPagamentosDashboard(); } catch(_) {}
        }
      }
    });
  } catch (e) {
    console.error('[sheetCancelarAgendamento]', e);
...
function adicionarBlocoServicoComDados(dados) {
  ...
  wrapper.innerHTML =
    ...
    '<div class="form-row">' +
    '<div class="form-group svc-prof-group"><label>Profissional</label>' +
    '<select class="svc-profissional" style="display:none" required><option value="">Selecione...</option></select>' +
    '<div class="svc-prof-custom-select">' +
      '<div class="svc-prof-trigger" onclick="toggleProfDropdown(this)">' +
        '<div class="svc-prof-trigger-content"><span class="svc-prof-placeholder">Selecione...</span></div>' +
        '<i class="fa-solid fa-chevron-down svc-prof-arrow"></i>' +
      '</div>' +
      '<div class="svc-prof-dropdown"></div>' +
    '</div>' +
    '</div>' +
  ...
  // Populate custom visual dropdown
  populateProfCustomDropdown(wrapper);
  ...
  if (currentUser.role === 'colaborador' && currentUser.profissionalNome) {
    ...
    // Sync visual trigger & lock it
    syncProfTriggerDisplay(wrapper);
    var __trigger = wrapper.querySelector('.svc-prof-trigger');
    if (__trigger) { __trigger.style.pointerEvents = 'none'; __trigger.style.opacity = '0.7'; }
...
/* ══════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════ */
function _profAvatarHtml(name, size) {
  size = size || 32;
  var url = (professionalAvatars && professionalAvatars[name]) || '';
  if (url) {
    return '<img src="' + url + '" alt="' + name + '" class="svc-prof-avatar" style="width:' + size + 'px;height:' + size + 'px;">';
  }
  // Fallback: iniciais com cor
  var initials = name.split(' ').map(function(w){return w.charAt(0).toUpperCase();}).slice(0,2).join('');
  var color = (profColors && profColors[name]) || '#6C3AED';
  return '<div class="svc-prof-avatar svc-prof-avatar-fallback" style="width:' + size + 'px;height:' + size + 'px;background:' + color + ';">' + initials + '</div>';
}

function populateProfCustomDropdown(wrapper) {
  var dropdown = wrapper.querySelector('.svc-prof-dropdown');
  if (!dropdown) return;
  dropdown.innerHTML = '';
  Object.keys(professionals).forEach(function(name) {
    var item = document.createElement('div');
    item.className = 'svc-prof-option';
    item.setAttribute('data-value', name);
    item.innerHTML = _profAvatarHtml(name, 36) + '<span class="svc-prof-option-name">' + name + '</span>';
    item.onclick = function(e) {
      e.stopPropagation();
      selectProfOption(wrapper, name);
    };
    dropdown.appendChild(item);
  });
}

function selectProfOption(wrapper, name) {
  var profSelect = wrapper.querySelector('.svc-profissional');
  var trigger = wrapper.querySelector('.svc-prof-trigger-content');
  var dropdown = wrapper.querySelector('.svc-prof-dropdown');
  var customSelect = wrapper.querySelector('.svc-prof-custom-select');

  // Set hidden select value
  profSelect.value = name;

  // Update trigger to show selected prof with avatar
  trigger.innerHTML = _profAvatarHtml(name, 28) + '<span class="svc-prof-selected-name">' + name + '</span>';

  // Close dropdown
  if (customSelect) customSelect.classList.remove('open');

  // Mark selected item
  dropdown.querySelectorAll('.svc-prof-option').forEach(function(o){
    o.classList.toggle('selected', o.getAttribute('data-value') === name);
  });

  // Fire the original change handler
  onSvcProfChange(profSelect);
}

function toggleProfDropdown(triggerEl) {
  var customSelect = triggerEl.closest('.svc-prof-custom-select');
  if (!customSelect) return;
  // Close any other open dropdowns
  document.querySelectorAll('.svc-prof-custom-select.open').forEach(function(el){
...
document.addEventListener('click', function(e) {
  if (!e.target.closest('.svc-prof-custom-select')) {
    document.querySelectorAll('.svc-prof-custom-select.open').forEach(function(el){
...
function syncProfTriggerDisplay(wrapper) {
  var profSelect = wrapper.querySelector('.svc-profissional');
  var trigger = wrapper.querySelector('.svc-prof-trigger-content');
  if (!profSelect || !trigger) return;
  var name = profSelect.value;
  if (name) {
    trigger.innerHTML = _profAvatarHtml(name, 28) + '<span class="svc-prof-selected-name">' + name + '</span>';
    var dropdown = wrapper.querySelector('.svc-prof-dropdown');
    if (dropdown) {
    ...
    trigger.innerHTML = '<span class="svc-prof-placeholder">Selecione...</span>';
  }
}
...
async function excluirAgendamento() {
  ...
  var ag = appointments.find(function(a) { return a.id === editingAppointmentId; });
  var tenantId = (typeof getCurrentTenantId === 'function') ? getCurrentTenantId() : (ag && ag.tenant_id) || null;
  var possuiPag = !!(ag && (ag.possui_pagamento || Number(ag.valor_total_pago || 0) > 0));
...
async function saveClient(e) {
  ...
  // Remove o prefixo "55" antes de validar comprimento (DDD + número = 10 ou 11 dígitos)
  var telDigits = telefoneRaw.replace(/\D/g, '').replace(/^55/, '');
  if (telDigits.length < 10 || telDigits.length > 11) {
    showToast('Telefone inválido. Informe DDD + número (10 ou 11 dígitos).', 'error');
    return;
  }
  // PADRÃO OFICIAL: persiste 55 + DDD + número (sem máscara) — pronto para Evolution API
  var telefone = normalizeTelefoneBR(telefoneRaw);
...
function openModalCriarProfissional() {
  ...
  var telNovo = document.getElementById('novo-prof-telefone');
  if (telNovo) {
    // Reset COMPLETO ao abrir o modal: limpa qualquer valor herdado e reaplica máscara
    telNovo.value = '';
    try { maskTelefoneProf(telNovo); } catch(e){}
    // maskTelefoneProf já chama apply() -> input passa a exibir "+55 "
  }
...
async function salvarNovoProfissional(e) {
  ...
  var telEl = document.getElementById('novo-prof-telefone');
  var telefoneRaw = telEl ? telEl.value.trim() : '';
  var telefone = normalizeTelefoneBR(telefoneRaw);
  var feedback = document.getElementById('criar-prof-feedback');
  if (!nome) { feedback.style.display = 'block'; feedback.style.color = '#e74c3c'; feedback.textContent = 'Nome é obrigatório!'; return; }
  // Espera 12 (DDD+8 dígitos) ou 13 (DDD+9 dígitos) com prefixo 55
  if (!telefone || (telefone.length !== 12 && telefone.length !== 13)) {
    feedback.style.display = 'block'; feedback.style.color = '#e74c3c';
    feedback.textContent = 'Telefone inválido! Informe DDD + número.';
    return;
  }
  ...
  var tenantId = getCurrentTenantId();
  var resp = await supabaseClient.from('profissionais').insert([{ nome: nome, telefone: telefone, foto_url: fotoUrl, tenant_id: tenantId }]).select();
...
function openModalEditarProfissional(profNome) {
  ...
  var telEdit = document.getElementById('edit-prof-telefone');
  if (telEdit) {
    // Ao carregar, normaliza o que veio do banco (ex: formato antigo "(48) 91203-769") e exibe com +55
    telEdit.value = formatTelefoneProfDisplay(prof.telefone || '');
    try { maskTelefoneProf(telEdit); } catch(e){}
  }
...
async function salvarEdicaoProfissional() {
  ...
  var telEl = document.getElementById('edit-prof-telefone');
  var telefoneRaw = telEl ? telEl.value.trim() : '';
  var telefone = normalizeTelefoneBR(telefoneRaw);
  if (!novoNome) { showToast('Nome é obrigatório!'); return; }
  if (telefoneRaw && telefone && telefone.length !== 12 && telefone.length !== 13) {
    showToast('Telefone inválido! Informe DDD + número.'); return;
  }
  var updateData = { nome: novoNome, telefone: telefone };
...
function playAppointmentSound() {
  ...
  try {
    if (!__appointmentAudio) {
      __appointmentAudio = new Audio(window.__beautyAssetUrl ? window.__beautyAssetUrl('sounds/appointment.mp3') : '/sounds/appointment.mp3');
    ...
    if (p && typeof p.catch === 'function') p.catch(function(err){ console.warn('[som] play bloqueado/falhou:', err && (err.name || err.message)); });
  } catch (err) {
    console.warn('[som] erro ao tocar confirmação:', err && (err.name || err.message));
  }
}
function playAppointmentSoundDebounced() {
  playAppointmentSound();
}
window.playAppointmentSound = playAppointmentSound;
window.playAppointmentSoundDebounced = playAppointmentSoundDebounced;
...
/* ============================================================
   🛡️ PACOTE-FIX (INTEGRIDADE) — bloqueio de exclusão de venda
   Se este agendamento contém origem='pacote_venda' e existirem
   OUTROS agendamentos consumindo créditos do(s) cliente_pacote(s)
   gerados aqui, NÃO permitir a exclusão.
   Retorna { ok:false, motivo, totalUsosExternos } ou { ok:true }.
   ============================================================ */
async function checarBloqueioExclusaoPacoteVenda(agendamentoId) {
  if (!agendamentoId) return { ok: true };
  var tenantId = getCurrentTenantId();
  try {
    var qVenda = supabaseClient.from('agendamento_servicos')
      .select('cliente_pacote_id')
      .eq('agendamento_id', agendamentoId)
      .eq('origem', 'pacote_venda')
      .not('cliente_pacote_id', 'is', null);
    if (tenantId) qVenda = qVenda.eq('tenant_id', tenantId);
    var rVenda = await qVenda;
    if (rVenda.error) {
      console.warn('[pacote][bloqueio-exclusao] busca venda falhou:', rVenda.error);
      return { ok: true }; // fail-open para não travar app por erro de leitura
    }
    var cpIds = [];
    (rVenda.data || []).forEach(function(r){
      if (r.cliente_pacote_id && cpIds.indexOf(r.cliente_pacote_id) === -1) cpIds.push(r.cliente_pacote_id);
    });
    ...
    if (rUsos.error) {
      console.warn('[pacote][bloqueio-exclusao] busca usos falhou:', rUsos.error);
      return { ok: true };
    }
    var rows = rUsos.data || [];
    ...
    if (totalAg > 0) {
      return {
        ok: false,
        motivo: 'Este pacote possui ' + totalAg + ' agendamento(s) utilizando seus créditos. ' +
                'Exclua primeiro os agendamentos de uso do pacote antes de remover a venda.',
        totalUsosExternos: totalAg
      };
    }
    return { ok: true };
  } catch (e) {
    console.warn('[pacote][bloqueio-exclusao] erro inesperado:', e);
    return { ok: true };
  }
}

deleteAppointment = async function(id) {
  // 🛡️ Bloqueio de integridade: venda de pacote com usos vinculados externos.
  var guard = await checarBloqueioExclusaoPacoteVenda(id);
  if (!guard.ok) {
    var err = new Error(guard.motivo);
    err.code = 'PACOTE_VENDA_BLOQUEADA';
    err.totalUsosExternos = guard.totalUsosExternos;
    throw err;
  }
...
saveAppointment = async function(e) {
  ...
  var __wasEditingAppointment = !!editingAppointmentId;
  try {
    showToast(__wasEditingAppointment ? 'Agendamento atualizado!' : 'Agendamento criado!', 'success');
  } catch (errToast) {
    console.error('[saveAppointment] showToast falhou:', errToast);
  }
  try {
    if (!__wasEditingAppointment && typeof window.playAppointmentSound === 'function') {
      window.playAppointmentSound();
    }
  } catch (errSound) {
    console.error('[saveAppointment] playAppointmentSound falhou:', errSound);
  }
...
async function openModalNovoPacote() {
  ...
  var dispExt = document.getElementById('pacote-disponivel-externo');
  if (dispExt) dispExt.checked = true; // default: true (novos pacotes visíveis no externo)
...
async function editarPacote(id) {
  ...
  var _dispExtEl = document.getElementById('pacote-disponivel-externo');
  if (_dispExtEl) {
    // Compatibilidade com pacotes antigos: se o campo for null/undefined, assume true.
    _dispExtEl.checked = (p.disponivel_agendamento_externo === false) ? false : true;
  }
...
async function salvarPacoteCrud(e) {
  ...
  var _dispExtEl2 = document.getElementById('pacote-disponivel-externo');
  var disponivelExterno = _dispExtEl2 ? !!_dispExtEl2.checked : true;
  ...
  var row = {
    ...
    ativo: ativo,
    disponivel_agendamento_externo: disponivelExterno,
...
async function carregarConfigGeral() {
  ...
  try {
    ...
    // === Automação de clientes inativos (notify-inactive-customers) ===
    var icAuto = false;
    if (resp && resp.data) {
      icAuto = !!resp.data.inactive_customer_automation_enabled;
    }
    var chkIcAuto = document.getElementById('cfg-inactive-customer-automation');
    if (chkIcAuto) chkIcAuto.checked = icAuto;

    // === Automação WhatsApp com Link Mágico (create-whatsapp-session) ===
    var waMagicOn = false;
    if (resp && resp.data) {
      waMagicOn = !!resp.data.whatsapp_magic_link_enabled;
    }
    var chkWaMagic = document.getElementById('cfg-whatsapp-magic-link');
    if (chkWaMagic) chkWaMagic.checked = waMagicOn;

    // === Cancelamento: exigir senha admin (flag) ===
    var exigirSenha = true; // fail-safe
    if (resp && resp.data && resp.data.exigir_senha_cancelamento != null) {
      var valor = resp.data.exigir_senha_cancelamento;
      exigirSenha = (
        valor === true ||
        valor === 'true' ||
        valor === 1 ||
...
/* ============================================================
   CANCELAMENTO — Feature flag (exigir senha admin)
   Persistido em tenant_settings.exigir_senha_cancelamento
   Default fail-safe = true (mantém comportamento atual).
   ============================================================ */
window.CANCELAMENTO_CFG = window.CANCELAMENTO_CFG || { exigir_senha: true };
var CANCELAMENTO_CFG = window.CANCELAMENTO_CFG;
...
/* ============================================================
   AUTOMAÇÃO DE CLIENTES INATIVOS (notify-inactive-customers)
   Persiste tenant_settings.inactive_customer_automation_enabled.
   Período de inatividade é FIXO em 30 dias na Edge Function.
   ============================================================ */
async function onToggleInactiveCustomerAutomation(el) {
  var tenantId = getCurrentTenantId();
  if (!tenantId) return;
  var on = !!el.checked;
  var fb = document.getElementById('cfg-inactive-feedback');
  el.disabled = true;
  try {
    var resp = await supabaseClient
      .from('tenant_settings')
      .upsert(
        { tenant_id: tenantId, inactive_customer_automation_enabled: on },
        { onConflict: 'tenant_id' }
      );
    if (resp.error) throw resp.error;
    if (fb) {
      ...
      fb.className = 'ac-feedback ok';
      fb.textContent = on
        ? '✓ Automação ativada. Mensagens serão enviadas diariamente para clientes inativos há 30+ dias.'
        : '✓ Automação desativada.';
      setTimeout(function(){ fb.style.display='none'; }, 4000);
    }
  } catch (err) {
    console.error('Erro ao salvar inactive_customer_automation_enabled:', err);
    el.checked = !on;
    if (fb) {
      fb.style.display = '';
      fb.className = 'ac-feedback err';
...
/* ============================================================
   AUTOMAÇÃO WHATSAPP COM LINK MÁGICO (create-whatsapp-session)
   Persiste tenant_settings.whatsapp_magic_link_enabled.
   Quando ativo, o webhook da Evolution dispara create-whatsapp-session,
   que gera token + envia link mágico. Quando desativado, a Edge Function
   encerra silenciosamente sem criar sessão nem enviar mensagem.
   ============================================================ */
async function onToggleWhatsappMagicLink(el) {
  var tenantId = getCurrentTenantId();
  if (!tenantId) return;
  var on = !!el.checked;
  var fb = document.getElementById('cfg-whatsapp-magic-feedback');
  el.disabled = true;
  try {
    var resp = await supabaseClient
      .from('tenant_settings')
      .upsert(
        { tenant_id: tenantId, whatsapp_magic_link_enabled: on },
        { onConflict: 'tenant_id' }
      );
    if (resp.error) throw resp.error;
    if (fb) {
      ...
      fb.className = 'ac-feedback ok';
      fb.textContent = on
        ? '✓ Automação ativada. Mensagens recebidas no WhatsApp dispararão o envio do link mágico.'
        : '✓ Automação desativada. A instância continua conectada, mas o link mágico não será mais enviado automaticamente.';
      setTimeout(function(){ fb.style.display='none'; }, 4000);
    }
  } catch (err) {
    console.error('Erro ao salvar whatsapp_magic_link_enabled:', err);
    el.checked = !on;
    if (fb) {
      fb.style.display = '';
      fb.className = 'ac-feedback err';
...
/* Toggle "Exigir senha no cancelamento" — salva imediatamente. */
async function onToggleExigirSenhaCancelamento(el) {
  var tenantId = getCurrentTenantId();
  if (!tenantId) return;
  var on = !!el.checked;
  var fb = document.getElementById('cfg-cancelamento-feedback');
  el.disabled = true;
  try {
    var resp = await supabaseClient
      .from('tenant_settings')
      .upsert(
        { tenant_id: tenantId, exigir_senha_cancelamento: on },
        { onConflict: 'tenant_id' }
      );
    if (resp.error) throw resp.error;
    CANCELAMENTO_CFG.exigir_senha = on;
    if (fb) {
      fb.style.display = '';
      fb.className = 'ac-feedback ok';
      fb.textContent = on
        ? '✓ Senha de administrador será exigida no cancelamento.'
        : '✓ Cancelamento não exigirá senha (apenas motivo).';
      setTimeout(function(){ fb.style.display='none'; }, 3000);
    }
  } catch (err) {
    console.error('Erro ao salvar exigir_senha_cancelamento:', err);
    el.checked = !on;
    if (fb) {
      fb.style.display = '';
      fb.className = 'ac-feedback err';
...
window.onToggleExigirSenhaCancelamento = onToggleExigirSenhaCancelamento;
...
function acceptUpsell(btn) {
  ...
  if (profEscolhido && profSelect) {
    ...
    var __wrapper8 = profSelect.closest('.servico-block');
    if (__wrapper8 && typeof syncProfTriggerDisplay === 'function') syncProfTriggerDisplay(__wrapper8);
...
function injectBloqueioBlock(container, b, hStart, isSingle, colIdx, totalCols) {
  ...
  // 📐 Bloqueio: posicionamento vertical considerando minutos
  var top = computeTopPx(Math.floor(iniMin / 60), iniMin % 60, hStart);
  var heightPxBlq = computeHeightPx(dur);
  ...
  block.style.height = heightPxBlq + 'px';
...
(function(){
  ...
  async function salvarProduto(e){
    ...
    var custoStr = (el('produto-custo') && el('produto-custo').value) || '';
    var custo = null;
    if (custoStr !== '' && custoStr != null) {
      var parsedCusto = parseFloat(String(custoStr).replace(',', '.'));
      if (isFinite(parsedCusto) && parsedCusto >= 0) {
        custo = parsedCusto;
      } else {
...
/* ============================================================
   DADOS CADASTRAIS DO TENANT (Configurações > Dados Cadastrais)
   - Carrega dados do tenant atual e popula formulário
   - Salva alterações na tabela `tenants`
   - Inclui máscara/CEP/ViaCEP
   ============================================================ */
(function dadosCadastraisTenantModule() {
  'use strict';

  var dcLogoFile = null;
  var dcLogoRemovido = false;

  function maskCepDc(input) {
    var v = (input.value || '').replace(/\D/g, '').substring(0, 8);
    if (v.length > 5)      input.value = v.substring(0,2) + '.' + v.substring(2,5) + '-' + v.substring(5);
    else if (v.length > 2) input.value = v.substring(0,2) + '.' + v.substring(2);
    else                   input.value = v;
  }

  function maskCpfCnpjDc(input) {
    var v = (input.value || '').replace(/\D/g, '').substring(0, 14);
    if (v.length <= 11) {
      // CPF
      if (v.length > 9)      input.value = v.substring(0,3)+'.'+v.substring(3,6)+'.'+v.substring(6,9)+'-'+v.substring(9);
      else if (v.length > 6) input.value = v.substring(0,3)+'.'+v.substring(3,6)+'.'+v.substring(6);
      else if (v.length > 3) input.value = v.substring(0,3)+'.'+v.substring(3);
      else                   input.value = v;
    } else {
      // CNPJ
  ...
  async function buscarCepDc(cepRaw) {
    var fb = document.getElementById('dc-cep-feedback');
    var digits = String(cepRaw || '').replace(/\D/g, '');
    if (fb) { fb.style.display = 'none'; fb.textContent = ''; }
    ...
    try {
      ...
      if (data.erro) {
        if (fb) { fb.style.display = 'block'; fb.textContent = 'CEP não encontrado.'; }
        return;
      }
      var lograd = document.getElementById('dc-logradouro');
      if (lograd) lograd.value = data.logradouro || lograd.value;
      var bairro = document.getElementById('dc-bairro');
      if (bairro) bairro.value = data.bairro || bairro.value;
      var cidade = document.getElementById('dc-cidade');
      if (cidade) cidade.value = data.localidade || cidade.value;
      var estado = document.getElementById('dc-estado');
      if (estado) estado.value = (data.uf || estado.value || '').toUpperCase();
    } catch (err) {
      if (fb) { fb.style.display = 'block'; fb.textContent = 'Não foi possível consultar o CEP. Preencha manualmente.'; }
    }
  }

  function bindDcInputs() {
    var cep = document.getElementById('dc-cep');
    if (cep && !cep.dataset.bound) {
      cep.addEventListener('input', function(){
        maskCepDc(this);
        var d = this.value.replace(/\D/g, '');
        if (d.length === 8) buscarCepDc(d);
      });
    ...
    var cpf = document.getElementById('dc-cpf-cnpj');
    if (cpf && !cpf.dataset.bound) {
      cpf.addEventListener('input', function(){ maskCpfCnpjDc(this); });
      cpf.dataset.bound = '1';
    }
    var tel = document.getElementById('dc-telefone');
    if (tel && !tel.dataset.maskProfBound && typeof maskTelefoneProf === 'function') {
      maskTelefoneProf(tel);
    }
  }

  function formatCepDisplay(d) {
    if (!d) return '';
    var s = String(d).replace(/\D/g,'').padStart(8,'0').substring(0,8);
  ...
  async function carregarDadosCadastrais() {
    ...
    bindDcInputs();
    try {
      var resp = await supabaseClient.from('tenants').select('*').eq('id', tenantId).maybeSingle();
      if (resp.error) { console.warn('[dc] erro ao carregar tenant:', resp.error.message); return; }
      var t = resp.data || {};

      setVal('dc-razao-social',  t.razao_social || t.nome || '');
      setVal('dc-nome-fantasia', t.nome_fantasia || t.nome || '');
      setVal('dc-cpf-cnpj',      t.cpf_cnpj || '');
      // Telefone: normalizado no banco (55XXXXXXXXXXX) -> exibe com máscara
      var telEl = document.getElementById('dc-telefone');
      if (telEl) {
        if (typeof formatTelefoneProfDisplay === 'function') {
          telEl.value = formatTelefoneProfDisplay(t.telefone || '');
        } else {
          telEl.value = t.telefone || '';
        }
      }
      setVal('dc-email', t.email || '');

      // Endereço
      setVal('dc-cep',         formatCepDisplay(t.cep));
      setVal('dc-logradouro',  t.logradouro || '');
      setVal('dc-numero',      t.numero || '');
      setVal('dc-complemento', t.complemento || '');
      setVal('dc-bairro',      t.bairro || '');
      setVal('dc-cidade',      t.cidade || '');
      setVal('dc-estado',      (t.estado || '').toUpperCase());

      // Logo
      var prev = document.getElementById('dc-logo-preview');
      var btnRem = document.getElementById('btn-remover-dc-logo');
      if (t.logo_url) {
        if (prev) prev.innerHTML = '<img src="' + t.logo_url + '" alt="Logo" style="max-width:100%;max-height:100%;object-fit:cover;">';
        if (btnRem) btnRem.style.display = '';
      } else {
        if (prev) prev.innerHTML = '<i class="fa-solid fa-image"></i>';
        if (btnRem) btnRem.style.display = 'none';
      }
      dcLogoFile = null;
      dcLogoRemovido = false;
    } catch (e) {
      console.error('[dc] exceção ao carregar tenant:', e);
    }
  }

  window.onDcLogoChange = function(input) {
    if (input.files && input.files[0]) {
      dcLogoFile = input.files[0];
      dcLogoRemovido = false;
      var reader = new FileReader();
      reader.onload = function(e) {
        var prev = document.getElementById('dc-logo-preview');
        if (prev) prev.innerHTML = '<img src="' + e.target.result + '" alt="Logo" style="max-width:100%;max-height:100%;object-fit:cover;">';
        var btnRem = document.getElementById('btn-remover-dc-logo');
        if (btnRem) btnRem.style.display = '';
      };
      reader.readAsDataURL(input.files[0]);
  ...
  window.removerDcLogo = function() {
    ...
    var btnRem = document.getElementById('btn-remover-dc-logo');
    if (btnRem) btnRem.style.display = 'none';
    var inp = document.getElementById('dc-logo-input');
    if (inp) inp.value = '';
  };
  ...
  window.salvarDadosCadastrais = async function() {
    var tenantId = (typeof getCurrentTenantId === 'function') ? getCurrentTenantId() : null;
    ...
    if (!tenantId) { showFb('Tenant não identificado.', '#DC2626'); return; }

    var razao  = (document.getElementById('dc-razao-social').value || '').trim();
    var fant   = (document.getElementById('dc-nome-fantasia').value || '').trim();
    var cpfCnp = (document.getElementById('dc-cpf-cnpj').value || '').trim();
    var telRaw = (document.getElementById('dc-telefone').value || '').trim();
    var email  = (document.getElementById('dc-email').value || '').trim();

    var telNorm = (typeof normalizeTelefoneBR === 'function') ? normalizeTelefoneBR(telRaw) : telRaw.replace(/\D/g,'');

    var cepDigits = (document.getElementById('dc-cep').value || '').replace(/\D/g,'');

    var payload = {
      razao_social:  razao || null,
      nome_fantasia: fant || null,
      nome:          fant || razao || null,
      cpf_cnpj:      cpfCnp || null,
      telefone:      telNorm || null,
      email:         email || null,
      cep:           cepDigits || null,
      logradouro:    (document.getElementById('dc-logradouro').value || '').trim() || null,
      numero:        (document.getElementById('dc-numero').value || '').trim() || null,
      ...
      estado:        ((document.getElementById('dc-estado').value || '').trim().toUpperCase().substring(0,2)) || null
    };
    ...
    try {
      ...
      if (dcLogoFile) {
        showFb('Enviando logo...');
        var logoUrl = await uploadDcLogo(dcLogoFile, tenantId);
        if (logoUrl) payload.logo_url = logoUrl;
      } else if (dcLogoRemovido) {
        payload.logo_url = null;
      }

      var upd = await supabaseClient.from('tenants').update(payload).eq('id', tenantId);
      if (upd.error) { showFb('Erro ao salvar: ' + upd.error.message, '#DC2626'); return; }

      showFb('Dados atualizados com sucesso!', '#16A34A');
      dcLogoFile = null;
      dcLogoRemovido = false;
      setTimeout(function(){ if (fb) fb.style.display = 'none'; }, 3000);

      if (typeof showToast === 'function') showToast('Dados cadastrais atualizados.', 'success');
    } catch (e) {
      showFb('Erro inesperado: ' + (e && e.message ? e.message : e), '#DC2626');
    }
  };

  // Carregar quando o usuário clicar na aba "Dados Cadastrais"
  document.addEventListener('click', function(ev){
    var btn = ev.target.closest && ev.target.closest('.config-tab[data-config-tab="dados-cadastrais"]');
    if (btn) {
      // Pequeno delay para garantir que painel ficou ativo
      setTimeout(function(){ try { carregarDadosCadastrais(); } catch(_) {} }, 50);
    }
  });

  // Expor para chamada manual / debugging
  window.carregarDadosCadastrais = carregarDadosCadastrais;
