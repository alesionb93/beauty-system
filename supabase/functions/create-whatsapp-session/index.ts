...
//   1) [NOVO] Garante idempotência por message_id (whatsapp_inbound_seen)
//   2) Cria uma sessão temporária na tabela whatsapp_sessions
//   3) Envia uma mensagem acolhedora com o link mágico via Evolution API
//
// Logs estruturados: prefixo [wa-session] em todas as etapas.
//
// CHANGELOG:
// - 2026-06-19: Idempotência por message_id. Evolution reenvia o
//   mesmo webhook quando há timeout, causando envio duplicado do
//   magic link. Agora cada message_id é gravado em
//   whatsapp_inbound_seen; reentregas retornam 200 sem processar.
// ============================================================
...
// ----- IDEMPOTÊNCIA: extrai message_id estável do payload Evolution
// Suporta tanto Evolution clássico (data.key.id) quanto Evolution Go
// (data.Info.ID). Retorna null se nada parseável (ex.: trigger manual).
...
Deno.serve(async (req) => {
  ...
  // ============================================================
  // 0) IDEMPOTÊNCIA — bloqueia reentregas do MESMO webhook
  // ------------------------------------------------------------
  // A Evolution reenvia o webhook quando nosso endpoint demora a
  // responder ou retorna erro. Sem este bloqueio, o cliente recebia
  // o magic link 2x ou 3x — gatilho clássico para o WhatsApp marcar
  // a conta como spam.
  //
  // Estratégia: PRIMARY KEY em message_id. Insert duplicado falha
  // com 23505 (unique_violation) e respondemos 200 OK sem fazer nada.
  //
  // Só aplica para payloads de webhook (têm body.data). Triggers
  // manuais (body.tenant_id direto) pulam a checagem.
  // ============================================================
  if (body?.data) {
    ...
    if (messageId) {
      ...
      if (seenErr) {
        ...
        // Outro erro (ex.: tabela ainda não criada): logar e seguir
        // para não quebrar o fluxo. Idempotência fica desativada
        // até a migration rodar.
        console.error(`${LOG_PREFIX} ${reqId} inbound_seen insert error (continuing)`, {
          error: seenErr.message,
          code: (seenErr as any).code,
        });
      } else {
        console.log(`${LOG_PREFIX} ${reqId} inbound message marked as seen`, {
          message_id: messageId,
        });
      }
    } else {
      console.log(`${LOG_PREFIX} ${reqId} no message_id in payload, skipping dedup`);
    }
  }
  ...
  // NOTA: `tenants` NÃO tem FK direta para `tenant_groups`.
  // A relação é N:N via `tenant_group_tenants` (group_id, tenant_id).
  ...
  // ----- 5) DEDUPLICAÇÃO (sessão ativa por telefone)
