...
async function loginAsAdmin(page) {
  return login(page, USERS.admin);
}

async function loginAsColaborador(page) {
  return login(page, USERS.colaborador);
}

async function loginAsMaster(page) {
  return login(page, USERS.master);
}

/**
 * Login utilizado pelos testes atuais do Slotify.
 */
async function loginSlotify(page) {
  ...
  await page
    .getByRole('textbox', { name: 'Login ou e-mail' })
    .fill('automacao');
  ...
  // Aguarda agenda carregar
  await page.waitForTimeout(5000);
}

/**
 * Garante que estamos na tela /agenda
 */
async function ensureOnAgenda(page) {
  if (page.url().includes('select-tenant')) {
    await page.click(
      '[data-testid="tenant-card"]:first-child, .tenant-card:first-child'
    );
...
/**
 * Logout via botão da UI.
 */
async function logout(page) {
  await page.click(
    '[data-testid="btn-logout"], #btn-logout, button:has-text("Sair")'
  );
