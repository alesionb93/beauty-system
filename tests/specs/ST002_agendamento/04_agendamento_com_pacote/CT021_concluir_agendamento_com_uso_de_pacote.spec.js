...
test('CT021 - Concluir agendamento com uso de pacote', async ({ page }) => {
  ...
  await test.step('📊 Profissional Daryl validado', async () => {
    ...
    await expect(linhaDaryl.locator('td:nth-child(4)')).toContainText('0');
    await expect(linhaDaryl.locator('td:nth-child(5)')).toContainText('0');
    await expect(linhaDaryl.locator('td.dash-prof-cell-caixinha')).toContainText('0');
    await expect(linhaDaryl.locator('td.dash-prof-cell-total-receber')).toContainText('0');
  });
  ...
  await test.step('📦 Pacote atualizado validado', async () => {
    ...
    log.info('Pacote atualizado para 2/4 utilizado');
  });
