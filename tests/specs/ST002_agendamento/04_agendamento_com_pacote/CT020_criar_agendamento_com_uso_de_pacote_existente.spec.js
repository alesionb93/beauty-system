...
test('CT020 - Criar agendamento com uso de pacote existente', async ({ page }) => {
  ...
  await test.step('📦 Pacote disponível validado e selecionado', async () => {
    ...
    await expect(
      cardPacote
    ).toContainText('3 restantes');

    await pacoteUso.check();

    log.info(
      'Pacote barba x4 utilizado (saldo atual 3)'
    );
  ...
  await test.step('📦 Saldo do pacote preservado', async () => {
    ...
    const listaPacotes = panePacotes
      .locator('ul.historico-lista');
    ...
    log.info(
      'Saldo mantido em 1/4 utilizado e 3 restantes'
    );
